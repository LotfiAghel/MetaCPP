...
namespace metacpp {
    ...
    void MetaExporter::Export(const std::string& inputSource, const std::string& outputHeader, const std::string& outputSource,const std::string& outputJson)
	{
		std::ofstream out_header(outputHeader);
		std::ofstream out_source(outputSource);
        std::ofstream out_json(outputJson);
        ...
        out_json << GenerateJsonFile({ outputJson }, storageData);
	}
    ...
    std::string metacpp::MetaExporter::GenerateJsonFile(const std::vector<std::string> &includes, mustache::data &storageData)
    {
        mustache::mustache source_template(templates::source);
        source_template.set_custom_escape([](const std::string& str) { return str; });
